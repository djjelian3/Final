...
function renderAdminShellStart(activeKey) {
  ...
  document.getElementById('admin-mount').innerHTML = `
    <div class="mobile-topbar">
      <button id="side-toggle" aria-label="Open admin menu" aria-expanded="false">${svg('menu')}</button>
  ...
  const setMenuState = (open) => {
    ...
    sideToggle.setAttribute('aria-label', open ? 'Close admin menu' : 'Open admin menu');
  };
  if (sideToggle) sideToggle.addEventListener('click', () => setMenuState(!sidebar.classList.contains('open')));
